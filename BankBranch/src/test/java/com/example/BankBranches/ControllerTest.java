...
public class ControllerTest
{
    ...
    void testAddBranch() throws Exception {
        ...
        // Expected entity (what service should return)
        BankBranch expected = new BankBranch();
        expected.setId(1L);
        expected.setBankName(dto.getBankName());
        expected.setBankBranchName(dto.getBankBranchName());
        expected.setBranchNumber(dto.getBranchNumber());
        expected.setDescription(dto.getDescription());
        expected.setBankBranchType(dto.getBankBranchType());
        expected.setCountryName(dto.getCountryName());
        expected.setOpenTime(dto.getOpenTime());
        expected.setCloseTime(dto.getCloseTime());
        expected.setWeeklyHolidayList(dto.getWeeklyHolidays());

        // Relaxed argument matching
        Mockito.when(bankBranchMethodsImplementation.saveBankBranch(Mockito.any(BankBranchDTO.class)))
                .thenReturn(expected);
        ...
        mockMvc.perform(post("/api/branches/addbranch")
                ...
                .andExpect(jsonPath("$.weeklyHolidays[0]").value("Sunday"));
    }
